...
- [ ] Define buck-native kernel target that builds PTX with the bootstrap toolchain (no external `build.sh`), reusing/updating the nvptx platform/target spec.
- [ ] Ensure libc swap for PTX is wired correctly via fixups (and any needed std/alloc tweaks) so the kernel links against `libc-hostcall`.
- [ ] Hook host binary to depend on the buck-built PTX artifact (no env/manual pathing), replacing the current `genrule` plumbing.
